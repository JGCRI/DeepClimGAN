...
class Discriminator(nn.Module):
	def __init__(self, label_smoothing, is_autoencoder, z_shape):
		...
		self.model = nn.Sequential(
			...
			#since we are doing conditional GAN (1 channels is for average maps)
			#conv3d(n_channels, 128),
			conv3d_same(n_channels, 128),
			lrelu(0.2),
			conv3d_same(128, 256),
	...
	def forward(self, x):
		...
		logging.info("b : {}, h: {}, w: {}, t: {}, ch: {}".format(b, h, w, t, ch))
		out = out.view(b, h * w * t * ch) #in my case h * w * t * ch = 4096				
		logging.info("out shape : {}".format(out.shape))
		lin1 = self.fc1(out)
		#lin2 = self.fc2(lin1)
