...
class Generator(nn.Module):
	def __init__(self, h, w, t, ch, batch_size, z_shape,  last_layer_size):
		...
		self.channels_in_avg_map = n_channels
		self.pool = pool2d()
		self.relu = relu()
		...
		self.batchNorm5d_5 = batchNorm5d(32+self.channels_in_avg_map+init_ctxt_channel_size)
		...
		self.upconv6 = conv3d_same(init_ctxt_channel_size+self.channels_in_avg_map+32, last_layer_size)
		...
		self.conv2_4_one_channel = conv2d(1, 4)
	...
	def forward(self, x, avg_context, high_res_context):
		...
		avg1 = self.pool(self.conv16_32(self.pool(self.conv8_16(self.pool(self.conv4_8(self.pool(self.conv2_4_one_channel(avg_context))))))))
		...
		avg2 = self.pool(self.conv8_16(self.pool(self.conv4_8(self.pool(self.conv2_4_one_channel(avg_context))))))
		...
		avg3 = self.pool(self.conv4_8(self.pool(self.conv2_4_one_channel(avg_context))))
		...
		avg4 = self.pool(self.conv2_4_one_channel(avg_context))
		...
		#less_than_zero = (out[:,pr_idx] < 0).sum().item()
		#logging.info("less than zero :{}".format(less_than_zero))
