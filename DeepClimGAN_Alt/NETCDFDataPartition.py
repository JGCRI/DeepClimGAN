...
class NETCDFDataPartition(data.Dataset):
	...
	def __init__(self,partition,data_dir,h,w,k,device):
		"""
		Init the dataset
	...
	def __getitem__(self, ids):
		...
		current_month = train[:n_channels, :, : , idx:(idx + n_days)] #output size is N_channels x H x W x 32
		high_res_context = train[:n_channels, :, :, (idx - context_window):idx]
		#get context
	...
	def build_input_for_D(self, input,real,  avg_context, high_res_context, train_with_lowres, train_with_highres):
		"""
		...
		mean_maps_for_real = real.mean(-1)
		mean_maps_for_input = input.mean(-1)
	...
	def get_noise(self, N, batch_size):
		...
		return self.m.sample(sample_shape=[batch_size,N])
